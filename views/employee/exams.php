          ...
          <div class="flex items-center justify-between">
            <div>
              <div class="flex items-center gap-3 mb-4">
                ...
                <h1 class="text-4xl font-bold"><?= __('employee.exams.available_exams') ?></h1>
              </div>
              <p class="text-white/90 text-lg"><?= __('employee.exams.test_knowledge') ?></p>
            </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
      <div class="card animate-fade-in">
        <div class="card-body">
          ...
          <div class="text-base text-gray-500"><?= __('employee.exams.available_count') ?></div>
        </div>
      </div>
      ...
      <div class="card animate-fade-in animate-delay-100">
        <div class="card-body">
          ...
          <div class="text-base text-gray-500"><?= __('employee.exams.completed_exams') ?></div>
        </div>
      </div>
      ...
      <div class="card animate-fade-in animate-delay-200">
        <div class="card-body">
          ...
          <div class="text-base text-gray-500"><?= __('employee.exams.average_results') ?></div>
        </div>
      </div>
      ...
      <div class="card animate-fade-in animate-delay-300">
        <div class="card-body">
          ...
          <div class="text-base text-gray-500"><?= __('employee.exams.earned_points') ?></div>
        </div>
      </div>
    </div>
    ...
    <div class="card mb-8">
      <div class="card-body">
        <h3 class="text-xl font-bold text-gray-900 mb-4 flex items-center">
          ...
          <?= __('employee.exams.filter_exams') ?>
        </h3>
        <div class="grid md:grid-cols-4 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2"><?= __('employee.exams.search') ?></label>
            <div class="relative">
              <input type="text" id="searchInput" placeholder="<?= __('employee.exams.search_placeholder') ?>" 
                     class="form-input w-full pl-10 pr-4 py-3">
              <i class="ri-search-line absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"></i>
            </div>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2"><?= __('employee.exams.category') ?></label>
            <select id="categoryFilter" class="form-select w-full px-4 py-3">
              <option value=""><?= __('employee.exams.all_categories') ?></option>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2"><?= __('employee.exams.difficulty_level') ?></label>
            <select id="difficultyFilter" class="form-select w-full px-4 py-3">
              <option value=""><?= __('employee.exams.all_levels') ?></option>
              <option value="beginner"><?= __('employee.exams.beginner') ?></option>
              <option value="intermediate"><?= __('employee.exams.intermediate') ?></option>
              <option value="advanced"><?= __('employee.exams.advanced') ?></option>
            </select>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2"><?= __('employee.exams.status') ?></label>
            <select id="statusFilter" class="form-select w-full px-4 py-3">
              <option value=""><?= __('employee.exams.all_exams') ?></option>
              <option value="available"><?= __('employee.exams.available') ?></option>
              <option value="completed"><?= __('employee.exams.completed') ?></option>
              <option value="in_progress"><?= __('employee.exams.in_progress') ?></option>
            </select>
          </div>
        </div>
      </div>
    ...
    <div id="examsContainer">
              ...
              <div class="card-body">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                  <div class="flex items-center text-sm text-gray-500">
                    ...
                    <span><?= (int)($exam['duration'] ?? 30) ?> <?= __('employee.exams.minutes') ?></span>
                  </div>
                  <div class="flex items-center text-sm text-gray-500">
                    <i class="ri-question-line ml-2"></i>
                    <span><?= (int)($exam['questions_count'] ?? 10) ?> <?= (int)($exam['questions_count'] ?? 10) == 1 ? __('employee.exams.question') : __('employee.exams.questions') ?></span>
                  </div>
                  <div class="flex items-center text-sm text-gray-500">
                    <i class="ri-star-line ml-2"></i>
                    <span><?= (int)($exam['points'] ?? 50) ?> <?= __('employee.exams.points') ?></span>
                  </div>
                  <div class="flex items-center text-sm text-gray-500">
                    <i class="ri-user-line ml-2"></i>
                    <span><?= (int)($exam['attempts'] ?? 0) ?> <?= (int)($exam['attempts'] ?? 0) == 1 ? __('employee.exams.attempt') : __('employee.exams.attempts') ?></span>
                  </div>
                </div>
                ...
                <?php if (($exam['status'] ?? 'available') === 'completed' && isset($exam['last_score'])): ?>
                  <div class="mb-4">
                    <div class="flex items-center justify-between text-sm mb-1">
                      <span class="text-gray-600"><?= __('employee.exams.last_score') ?></span>
      ...
      <?php else: ?>
        <div class="text-center py-16">
          ...
          <h3 class="text-xl font-medium text-gray-900 mb-2"><?= __('employee.exams.no_exams_available') ?></h3>
          <p class="text-gray-500 mb-6"><?= __('employee.exams.no_exams_message') ?></p>
          ...
          <a href="<?= $basePath ?>/dashboard" class="inline-flex items-center px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
            ...
            <?= __('employee.exams.back_to_dashboard') ?>
          </a>
        </div>
      <?php endif; ?>
    </div>
