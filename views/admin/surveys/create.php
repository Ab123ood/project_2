...
<div class="bg-white shadow-sm border border-gray-200 rounded-xl mb-6">
  <div class="px-6 py-4 flex items-center justify-between">
    <div class="flex items-center">
      ...
      <div>
        <h1 class="text-xl md:text-2xl font-bold text-gray-900"><?= __('admin.surveys.create_title') ?></h1>
        <p class="text-sm text-gray-600"><?= __('admin.surveys.description') ?></p>
      </div>
    </div>
    <a href="<?= $basePath ?>/admin/surveys" class="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50"><?= __('common.back') ?></a>
  </div>
</div>
...
<form id="surveyForm" method="post" action="<?= $basePath ?>/admin/surveys" class="max-w-6xl mx-auto">
  ...
  <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
    <div class="flex items-center mb-6">
      ...
      <h2 class="text-lg font-medium text-gray-900"><?= __('admin.surveys.form.basic_info') ?></h2>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2"><?= __('admin.surveys.form.title') ?></label>
        <input type="text" id="surveyTitle" name="title" value="<?= htmlspecialchars($old['title'] ?? '') ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" placeholder="<?= __('admin.surveys.form.title_placeholder') ?>" required>
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-2"><?= __('admin.surveys.form.category_label') ?></label>
        ...
        <select id="surveyCategory" name="category" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
          <option value="" <?= $cat===''?'selected':''; ?>><?= __('admin.surveys.form.choose_category') ?></option>
